...
static ViewFiller fillers[] = 
{
    ...
    /*  4 */ [](String &fill) { char buff[8]; sprintf(buff, "%ld", (t - historyControl.getLastRecovery()) / 3600 / 24); fill = buff;  },
    /*  5 */ [](String &fill) { char buff[8]; sprintf(buff, "%ld", ((t - historyControl.getLastRecovery()) / 3600) % 24); fill = buff;  },
    /*  6 */ [](String &fill) { char buff[8]; sprintf(buff, "%ld", ((t - historyControl.getLastRecovery()) / 60) % 60); fill = buff;  },
